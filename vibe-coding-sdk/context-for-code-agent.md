...
- Prompts: every route loads the latest `agent_prompts` (columns `agent_name`, `prompt_type`, `language`, `content`, `updated_at`) with `prompt_type='system'`; missing rows throw a hard error so your deployment fails fast if the prompt isn't provisioned. Agents: `default_triage` (ask what the user needs; explain onboarding/campaign/feedback/setter options), `onboarding` (collect profile), `campaign`/`feedback` (same agent: handles campaign proposals and feedback loops via pub/sub), `setter` (qualify leads and set next steps). Master-agent now resolves agents by `agent_name`/`name`/`slug` to fit existing Supabase schema.
- Conversation persistence: `lib/state.js` uses atomic upsert (`upsertConversationByAgentAndPhone`) to handle concurrent requests without race conditions. The unique constraint on `(agent_id, phone_number)` ensures only one conversation exists per agent+phone combination. Kapso metadata (external ID, phone, owner, contact) is recorded so repeated messages append to the same Supabase conversation ID.
- Race condition fix (2025-12-09): Replaced check-then-insert pattern with database-level upsert to eliminate duplicate key errors (`23505`) when concurrent webhooks arrive for the same agent+phone. No more race condition recovery logging.
- Logging: structured logs with request IDs trace complete request lifecycle: `[request-start]`, `[conversation-upsert]`, `[cache-hit/miss]`, `[agent-switch]`, `[parser-decision/calling/success/skipped]`, `[request-complete]`. Each request logs `[master-agent] version=X.X.X revision=... method=...`.
...
- Testing: `npm test` runs all tests including race condition validation with concurrent requests; see `tests/inbound.test.js` for parser gating and conversation reuse tests.
