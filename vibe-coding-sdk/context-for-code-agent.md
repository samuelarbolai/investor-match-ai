...
- Prompts: every route loads the latest `agent_prompts` (columns `agent_name`, `prompt_type`, `language`, `content`, `updated_at`) with `prompt_type='system'`; missing rows throw a hard error so your deployment fails fast if the prompt isn’t provisioned. Agents: `default_triage` (ask what the user needs; explain onboarding/campaign/feedback/setter options), `onboarding` (collect profile), `campaign`/`feedback` (same agent: handles campaign proposals and feedback loops via pub/sub), `setter` (qualify leads and set next steps).
